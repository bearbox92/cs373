...
def get_opt_range (i : int, j: int) -> (int, int):
    median = (j/2) + 1
    if i < median:
      i = median
    return (i, j)
  
def get_cycle_length(k : int) -> int :
    count = 1
    while k > 1:
      if k % 2 == 0:
        k = k//2
...
def eval_cycle_length (i: int, j: int) -> int:
    max_cycle_length = 0
    for l in range(i, j + 1):
      cycle_length = get_cycle_length(l)
      if cycle_length > max_cycle_length:
        max_cycle_length = cycle_length
    return max_cycle_length
  
def get_max_cache (i: int, j: int) -> int:
    max_cycle_length = 0
    for k in range (i, j+1):
      if Collatz_cache[k] > max_cycle_length:
        max_cycle_length = Collatz_cache[k]
    return max_cycle_length
...
def collatz_eval (i: int, j: int) -> int :
  ...
  # Check range
      ...
      i, j = get_opt_range(i,j)
      max_cycle_length = 0
      if j-i <= 1000:
        return eval_cycle_length(int(i), int(j))
      else:
        range_i = ((i//1000) +1) * 1000
        range_j = (j//1000) * 1000
        a = eval_cycle_length (int(i), int(range_i))
        b = eval_cycle_length (int(range_j), int(j))
        c = get_max_cache (int(range_i//1000), int(range_j // 1000))
        
        max_cycle_length = get_max(a,b,c)
      return max_cycle_length
